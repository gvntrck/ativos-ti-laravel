...
@section('header', $computer->hostname)
@section('subheader', ucfirst($computer->type) . ' - ' . $computer->asset_code)
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">Detalhes do Equipamento</h3>
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        <div class="sm:col-span-1">
                            <dt class="text-sm font-medium text-gray-500">Hostname</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $computer->hostname }}</dd>
                        </div>
                        <div class="sm:col-span-1">
                        ...
                        <div class="sm:col-span-1">
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $computer->user_name ?? '-' }}</dd>
                        </div>
                        <div class="sm:col-span-1">
                            <dt class="text-sm font-medium text-gray-500">Localização</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $computer->location ?? '-' }}</dd>
                        </div>
                        <div class="sm:col-span-1">
                            <dt class="text-sm font-medium text-gray-500">Patrimônio</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $computer->property ?? '-' }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                            <dt class="text-sm font-medium text-gray-500">Especificações</dt>
                            <dd class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ $computer->specs ?? '-' }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                            <dt class="text-sm font-medium text-gray-500">Observações</dt>
                            <dd class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ $computer->notes ?? '-' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <!-- Add History Form -->
                    <form action="{{ route('computers.history.store', $computer) }}" method="POST"
                    ...
                    <div class="flow-root">
                        <ul class="-mb-8">
                            @foreach($computer->history as $history)
                                <li>
        ...
        <div class="md:col-span-1 space-y-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @if($computer->photo_url)
                        <img src="{{ $computer->photo_url }}" alt="Foto do computador"
